...
import org.nasdanika.html.app.legacy.impl.Application;
import org.nasdanika.html.app.legacy.impl.BootstrapContainerApplication.Section;
	...
	protected Function<Map<EStructuralFeature, java.util.function.Consumer<BootstrapElement<?,?>>>, Tag> createContainerFunction(Context context) {
		return new Function<Map<EStructuralFeature, java.util.function.Consumer<BootstrapElement<?,?>>>, Tag>() {
			...
			public Tag execute(Map<EStructuralFeature, java.util.function.Consumer<BootstrapElement<?,?>>> inputs, ProgressMonitor progressMonitor) throws Exception {
				...
				Col header = inputs.containsKey(AppPackage.Literals.PAGE__HEADER) ? container.row().col() : null;				
				if (header != null) {
					header.toHTMLElement().addClass("nsd-app-header");
					inputs.get(AppPackage.Literals.PAGE__HEADER).accept(header);
				}
				
				Col navigationBar = inputs.containsKey(AppPackage.Literals.PAGE__NAVIGATION_BAR) ? container.row().col() : null;
				
				navigationBar.toHTMLElement().addClass("nsd-app-navbar");

				Row contentRow = inputs.containsKey(AppPackage.Literals.PAGE__NAVIGATION_PANEL) || inputs.containsKey(AppPackage.Literals.PAGE__CONTENT_PANEL) ? container.row() : null;
				...
				Col footer = inputs.containsKey(AppPackage.Literals.PAGE__FOOTER) ? container.row().col() : null;
