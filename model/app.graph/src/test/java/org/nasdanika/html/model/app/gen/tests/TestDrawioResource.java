...
import org.nasdanika.html.model.app.graph.AppAdapterFactory;
import org.nasdanika.html.model.app.graph.Util;
		...
		BiConsumer<File, String> listener = new BiConsumer<File, String>() {
			...
			public void accept(File file, String path) {
				if (path.endsWith(".html")) {
					...
					try {	
						Predicate<String> predicate = org.nasdanika.html.model.app.graph.Util.createRelativeLinkPredicate(file, siteDir);						
						Consumer<? super Element> inspector = org.nasdanika.html.model.app.graph.Util.createInspector(predicate, error -> {
							System.err.println("[" + path +"] " + error);
							problems.incrementAndGet();
						});
						JSONObject searchDocument = org.nasdanika.html.model.app.graph.Util.createSearchDocument(path, file, inspector, null, e -> problems.incrementAndGet());
