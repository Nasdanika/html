...
[org.nasdanika.html](apidocs/index.html) bundle provides [fluent API](https://en.wikipedia.org/wiki/Fluent_interface) for building HTML code including:
...
* An instance of [HTMLFactory](apidocs/index.html?org/nasdanika/html/HTMLFactory.html) is used to create instances of API interfaces.
...
In applications where HTML API is used out of the context of a web request [DefaultHTMLFactory](apidocs/index.html?org/nasdanika/html/impl/DefaultHTMLFactory.html) class can be directly instantiated and used as HTMLFactory implementation. In a context of a web request, e.g. in routes or route operations, and instance of HTMLFactory can be obtained either by adapting [HttpServletRequestContext](http://www.nasdanika.org/server/apidocs/org.nasdanika.web/target/site/apidocs/org/nasdanika/web/HttpServletRequestContext.html) or by specifying a context parameter - in this case the framework will adapt the context to HTMLFactory.

Many HTML interfaces extend [Producer](apidocs/index.html?org/nasdanika/html/Producer.html) and [AutoCloseable](http://docs.oracle.com/javase/8/docs/api/java/lang/AutoCloseable.html). Many API methods take objects as arguments to build a composite HTML object from parts.
...
Objects are converted to HTML string (stringified) using the following algorithm (see. [UIElementImpl.toHTML()](apidocs/index.html?src-html/org/nasdanika/html/impl/UIElementImpl.html#line.1063) source code):  
...
* If object implements [FactoryProducer](apidocs/index.html?org/nasdanika/html/FactoryProducer.html), then its ``produce(HTMLFactory)`` method is invoked and return value is recursively passed to stringification. 
...
## Low level API
Low level HTML API provide means to construct HTML elements and manipulate their content and attributes. Higher level API implementations are built leveraging low-level API. The base interface for most HTML element interfaces is [UIElement](apidocs/index.html?org/nasdanika/html/UIElement.html). HTML [Tag](apidocs/index.html?org/nasdanika/html/Tag.html) can be created using ``tag(String, Object...)`` and ``tag(TagName, Object...)`` methods of HTMLFactory, where [TagName](apidocs/index.html?org/nasdanika/html/Tag.TagName.html) is an enumeration of tag names. 
...
``input(InputType)`` method can be used to create [input](apidocs/index.html?org/nasdanika/html/Input.html) elements. [InputType](apidocs/index.html?org/nasdanika/html/HTMLFactory.InputType.html) is an enumeration of input types. [TextArea](apidocs/index.html?org/nasdanika/html/TextArea.html) and [Select](apidocs/index.html?org/nasdanika/html/Select.html) are created by ``textArea()`` and ``select()`` methods respectively.

``fragment(Object...)`` method creates a [Fragment](apidocs/index.html?org/nasdanika/html/Fragment.html), which is a collection of HTML elements which can be operated on as single unit.

Some other low-level factory methods include ``link(Object, Object...)``, ``ol(Iterable<?>)``, and ``ul(Iterable<?>)``.

## CSS Styles

CSS styles can be set with ``style(String,Object)`` method or with UIElement ``style()`` method which returns [Style](apidocs/index.html?org/nasdanika/html/Style.html) interface providing bindings for frequently used styles, e.g.
...
Bootstrap classes can be set on any UIElement by invoking ``bootstrap()`` method and then one of [Bootstrap](apidocs/index.html?org/nasdanika/html/Boostrap.html) interface methods, e.g.:
...
### Forms
Bootstrap forms functionality is available through [Form](apidocs/index.html?org/nasdanika/html/Form.html) interface. Inputs shall be created by 
HTMLFactory and then added to [FormGroup](apidocs/index.html?org/nasdanika/html/FormGroup.html), [FormInputGroup](apidocs/index.html?org/nasdanika/html/FormInputGroup.html) or [InputGroup](apidocs/index.html?org/nasdanika/html/InputGroup.html) by invoking respective Form methods.

### Glyphicons
[Glyphicons](apidocs/index.html?org/nasdanika/html/Bootstrap.Glyphicon.html) enumeration contains a list of [Bootstrap glyphs](http://getbootstrap.com/components/#glyphicons) and ``glyphicon(Glyphicon)`` method can be used to create a glyph:
...
``fontAwesome()`` factory method is a shortcut for ``span().fontAwesome()``. [FontAwesome.Stack](apidocs/index.html?org/nasdanika/html/FontAwesome.Stack.html) interface is used for creating [stacked icons](https://fortawesome.github.io/Font-Awesome/examples/#stacked), it can be created with the factory method ``fontAwesomeStack()``.

## AngularJS
[AngularJS](https://angularjs.org/) directives can be added to UIElement's by invoking ``angular()`` method and then one of methods of returned [Angular](apidocs/index.html?org/nasdanika/html/Angular.html) instance:
...
## Knockout
Similarly to AngularJS directives, [Knockout](http://knockoutjs.com/index.html) bindings can be added to UIElement's by invoking ``knockout()`` methods and then of of methods of returned [Knockout](apidocs/index.html?org/nasdanika/html/Knockout.html) instance:

```java
myButton.knockout().click("doCoolStuff()")
```

[Knockout virtual elements](apidocs/index.html?org/nasdanika/html/KnockoutVirtualElement.html) can be created with the factory's ``knockoutVirtualElement(Object... content)`` method.

## Templates and interpolation
HTMLFactory supports simple templating two [interpolate](apidocs/index.html?org/nasdanika/html/HTMLFactory.html#interpolate-java.lang.Object-org.nasdanika.html.HTMLFactory.TokenSource-) methods which expand ``{{token name}}`` tokens in the source. Source is stringified before expansion using the algorithm explained above.
...
``applicationPanel()`` factory methods creates an instance of [ApplicationPanel](apidocs/index.html?org/nasdanika/html/ApplicationPanel.html), which provides a convenient way to create web pages with a header, navigation bar, several content panels and a footer.
...
``bootstrapRouterApplication()`` method is similar to ``routerApplicationMethod()`` but allows to specify a UI [Theme](apidocs/index.html?org/nasdanika/html/Theme). [Nasdanika Application Workspace Wizard](Home) generates applications with the default Bootstrap theme and themes from [Bootswatch](https://bootswatch.com/).
